...
class Database():
    ...
    def dbreg(self, *arg_content):
        self.cursor.execute(f'INSERT INTO users VALUES (?,?,?,?,?)', (arg_content[0], arg_content[1], arg_content[2], arg_content[3], arg_content[4]))
        self.conn.commit()
...
def start_message(message):
    ...
    if data is None:
        if message.from_user.username is None:
            db.dbreg(message.chat.id, message.from_user.first_name, 1000, 100, 100)
        else:
            db.dbreg(message.chat.id, message.from_user.username, 1000, 100, 100)
